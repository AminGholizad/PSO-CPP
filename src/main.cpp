...
[[nodiscard]] constexpr pso::Cost cost_fcn(std::span<const double> variables) {
    pso::Cost cost{.objective = 0.0, .infeasibility = 0.0};
    for (const auto &var : variables) {
        cost.objective += std::sin(var * 5.0) + // NOLINT(readability-magic-numbers,
                                                // cppcoreguidelines-avoid-magic-numbers)
                          std::sin(var * 7.0) + // NOLINT(readability-magic-numbers,
                          // cppcoreguidelines-avoid-magic-numbers)
                          std::sin(var * 11.0); // NOLINT(readability-magic-numbers,
                                                // cppcoreguidelines-avoid-magic-numbers)
        cost.infeasibility += std::sin(var);
    }
    cost.objective = std::abs(cost.objective);
    cost.infeasibility = std::abs((cost.infeasibility / static_cast<double>(variables.size())) -
